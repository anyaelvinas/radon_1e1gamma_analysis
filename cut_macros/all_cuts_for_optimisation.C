...
// Macro to optimise a given cut
    ...
    // Cut 6: Timing cut
// This macro is to be used with pipelines/optimise_cutN.py

// To run for different cuts:
    // comment out the relevant threshold: lines 82-86
    // comment out the relevant lines in the loop 
...
void all_cuts_for_optimisation(const char* inputFileName, const char* outputFileName, double threshold_to_optimise) {
    // Load the dataset 
    ...
    const double y_max_pos = 2000;
    const double y_max_neg = -2000;
    const double x_max_pos = 435;
    const double x_max_neg = -435;

    // Thresholds to change
    const int num_gammas = 1; // 1 gamma (and 1 electron)
    const double min_E = 0.05; // 50keV
    const double max_E_tot = 3.27; // MeV
    //const double start_vertex_edge_buffer = 45.0; // mm from the edge of the calo walls/source foil
    const double t_threshold = 4.0; // ns 
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        if (!(electron_number == 1 && gamma_number == num_gammas)) continue;
        //if (electron_number != 1) continue; // TO COMMENT OUT WHEN NOT DOING CUT 1
        //if (gamma_number < 1 || gamma_number > (int)threshold_to_optimise) continue; // TO COMMENT OUT WHEN NOT DOING CUT 1
        pass_cut1++;
        ...
        //if (electron_energy < threshold_to_optimise || gamma_energy < threshold_to_optimise) continue; // TO COMMENT OUT WHEN NOT DOING CUT 2
        pass_cut2++;

        // CUT 3: total energy ≤ 3.27 MeV
        double E_tot = electron_energy + gamma_energy;
        if (E_tot > max_E_tot) continue;
        //if (E_tot > threshold_to_optimise) continue; // TO COMMENT OUT WHEN NOT DOING CUT 3
        pass_cut3++;

        // CUT 4: gamma OM number ≤ 512
        if (gamma_om_x->empty()) continue; // if there is no entries, skip
        if (om_number->at(0) > 512) continue; 
        pass_cut4++;

        // CUT 5: avoid electron start vertices in buffer zones
        // Dodge x buffer zones
        //if (first_vertex_x->at(e_idx) > -1*start_vertex_edge_buffer && first_vertex_x->at(e_idx) < start_vertex_edge_buffer) continue;
        if (first_vertex_x->at(e_idx) > -1*threshold_to_optimise && first_vertex_x->at(e_idx) < threshold_to_optimise) continue; // TO COMMENT OUT WHEN NOT DOING CUT 5
        //if (first_vertex_x->at(e_idx) > x_max_pos - start_vertex_edge_buffer || first_vertex_x->at(e_idx) < x_max_neg + start_vertex_edge_buffer) continue;
        if (first_vertex_x->at(e_idx) > x_max_pos - threshold_to_optimise || first_vertex_x->at(e_idx) < x_max_neg + threshold_to_optimise) continue; // TO COMMENT OUT WHEN NOT DOING CUT 5
        // Dodge y buffer zones
        //if (first_vertex_y->at(e_idx) > y_max_pos - start_vertex_edge_buffer || first_vertex_y->at(e_idx) > y_max_neg + start_vertex_edge_buffer) continue;
        if (first_vertex_y->at(e_idx) > y_max_pos - threshold_to_optimise || first_vertex_y->at(e_idx) > y_max_neg + threshold_to_optimise) continue; // TO COMMENT OUT WHEN NOT DOING CUT 5
        pass_cut5++;
        ...
        if (diff > t_threshold) continue; // reject event
        //if (diff > threshold_to_optimise) continue; // TO COMMENT OUT WHEN NOT DOING CUT 6
        pass_cut6++;
