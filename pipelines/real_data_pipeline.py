...
cut_macro = "/sps/nemo/scratch/elvin/macros/cuts_V1.C" # CHANGE THIS LINE WHEN I OPTIMISE THE CUTS
simulation_summary_csv = "/sps/nemo/scratch/elvin/csvs/simulation_summary.csv"
output_csv = "/sps/nemo/scratch/elvin/csvs/real_data_summary_V1.csv" # CHANGE NAME WHEN WORKING WITH ALTERED CUTS - right now its for V1
detector_vol = 15.4 
...
def calculate_activity(duration, N_cut):
    """
    ...
    # Calculate activity in mBq / m^3
    A = N_cut / (duration * detector_vol * efficiency)
    A_unc = 0.1*A # PLACEHOLDER - CHANGE
    print("\nActivity calculated!")
    return A, A_unc
...
def update_csv(run_number, time, duration, phase, N_orig, N_cut, cut_file, A, A_unc):
    ...
    # Create csv if not already made
        ...
        header = ["run", "time_Unix", "Duration_s", "Phase", "N_orig", "N_cut", "cut_file", "A_mBq_m-3", "Delta_A"]
        data_rows = []
...
def run_pipeline():
    ...
    for file in os.listdir(data_directory):
        ...
        A, A_unc = calculate_activity(duration, N_cut)
